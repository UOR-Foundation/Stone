...
export class ClaudeFileGenerator {
   ...
   */
  public async generateClaudeFiles(): Promise<void> {
    // Ensure directory exists
    const claudeDir = path.join(process.cwd(), this.config.github.stoneDirectory);
    ...
    if (this.config.roles.pm.enabled) {
      ...
      fs.writeFileSync(
        path.join(claudeDir, this.config.roles.pm.claudeFile),
        pmClaudeContent,
        'utf8'
      );
    }
    ...
    if (this.config.roles.qa.enabled) {
      ...
      fs.writeFileSync(
        path.join(claudeDir, this.config.roles.qa.claudeFile),
        qaClaudeContent,
        'utf8'
      );
    }
    ...
    if (this.config.roles.feature.enabled) {
      ...
      fs.writeFileSync(
        path.join(claudeDir, this.config.roles.feature.claudeFile),
        featureClaudeContent,
        'utf8'
      );
    }
    ...
    if (this.config.roles.auditor.enabled) {
      ...
      fs.writeFileSync(
        path.join(claudeDir, this.config.roles.auditor.claudeFile),
        auditorClaudeContent,
        'utf8'
      );
    }
    ...
    if (this.config.roles.actions.enabled) {
      ...
      fs.writeFileSync(
        path.join(claudeDir, this.config.roles.actions.claudeFile),
        actionsClaudeContent,
        'utf8'
      );
    }
  }
   ...
   */
  private generatePMClaudeFile(): string {
...
`;
   ...
   */
  private generateQAClaudeFile(): string {
    return `# QA Team Role
...
`;
   ...
   */
  private generateFeatureClaudeFile(): string {
...
`;
   ...
   */
  private generateAuditorClaudeFile(): string {
    return `# Auditor Role
...
`;
   ...
   */
  private generateActionsClaudeFile(): string {
